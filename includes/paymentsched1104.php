...
$havnew = mysql_num_rows($checknew);
        ...
        if ($havnew>0 and $plspass=='') { //may bago na line
                $nat=0;
                while ($newnew = mysql_fetch_assoc($checknew)) {
                        $nature[$nat] = $newnew[bus_code];
                        $nat++;
		$getmnth = date('m', strtotime($newnew[date_create]));

                }
        }
			if ($havnew>0  and $plspass==''){
			if ($getmnth > 0 and $getmnth <=3) {
				$getqqtr = 0;
			} elseif ($getmnth > 3 and $getmnth <=6) {
                                $getqqtr = 1;
                        } elseif ($getmnth > 6 and $getmnth <=9) {
                                $getqqtr = 2;
                        } elseif ($getmnth > 9 and $getmnth <=12) {
                                $getqqtr = 3;
                        }
				//get fee
				$bis=0;
			...
			$divfee = ($divfee / $gettqqtr) + $newfee;
			$newtax = $newtax / 4;
			echo "($divfee / $gettqqtr) + $newfee <br>";
			}
		echo "$newfee ===== $divtax + $divfee + $getsumnun1) - $paymade; <br>";
