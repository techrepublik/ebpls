...
//if ($stat != "New") {
	...
	if ($permit_type == "Business" and $grandamt != '0') {
		$getpref = @mysql_query("select * from ebpls_buss_preference");
		$getpref1 = @mysql_fetch_assoc($getpref);
		$nPerLine = $getpref1['sassess'];
		...
		$busint = 0;
		if ($nPerLine == "") {
			$nTotalTax = ($robertTax - (($robertTax * $getbusexempt1[tax_exemption]) / 100));
		...
		if ($datetoday > $renewaldate) {
			...
			$busvalues = "'', '$owner_id', '$business_id', '$nTaxnFee', '$buspen', '$busint', '0', '$nTotalTax', '', '1', '$stat', '$totexemption', '$yearnow1', 'ANNUAL', '1'";
