...
$pdf->Cell(190,5,'NOTICE OF BUSINESS TAX COLLECTION',0,2,'C');
...
$paymentmode = strtolower($resulta[14]);
...
$pdf->Cell(200,5,'The following are due for payment:',0,1,'L');
...
$gettax = mysql_query("select a.compval, b.tfodesc, c.payment_part, c.ts
 ...
 $i = 1;
        ...
        while ($busline=mysql_fetch_row($gettax))
        {
    		...
    		if ($paymentmode == 'semi-annual' and $periodico = '1') {
		        	$paymentmode = '1st half';
	    	} else {
		    		$paymentmode = '2nd half';
    		}
    		if ($paymentmode == 'quarterly' and $periodico = '1') {
		        	$paymentmode = '1st Quarter';
	    	}
	    	if ($paymentmode == 'quarterly' and $periodico = '2') {
		        	$paymentmode = '2nd Quarter';
	    	}
	    	if ($paymentmode == 'quarterly' and $periodico = '3') {
		        	$paymentmode = '3rd Quarter';
	    	}
            if ($paymentmode == 'quarterly' and $periodico = '4') {
		        	$paymentmode = '4th Quarter';
	    	}
	    	$pdf->SetX(5);
            $pdf->Cell(80,5,$busline[1],1,0,'L');
			$pdf->SetX(85);
			$pdf->Cell(40,5,$busline[3],1,0,'L');
			$pdf->SetX(125);
...
$report_desc='Notice of Business Tax Collection';
