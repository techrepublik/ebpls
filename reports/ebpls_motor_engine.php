...
//Page header
	...
	$this->Cell(340,5,'LISTING OF MOTOR VEHICLE BY ENGINE',0,1,'C');
	$this->Ln(22);
	}
...
/*	$result = mysql_query("select concat(a.owner_first_name, ' ', a.owner_middle_name, ' ', a.owner_last_name), 
	b.motorized_operator_permit_id, b.motorized_no_of_units, b.for_year, b.paid, b.pin,
	c.motorized_motor_model, c.motorized_motor_no, c.motorized_chassis_no, c.motorized_plate_no, c.motorized_body_no,
	c.route, c.linetype, c.permit_type, c.body_color, c.lto_number, c.cr_number
	from ebpls_mtop_owner a, ebpls_motorized_operator_permit b, ebpls_motorized_vehicles c, 
	tempbusnature d, ebpls_transaction_payment_or e
	where a.owner_id = b.owner_id and a.owner_id = c.motorized_operator_id") 
	or die(mysql_error());
...
*/
	...
	while ($resulta=mysql_fetch_row($result))
	{
		...
		$pdf->SetX(200);
		$pdf->Cell(30,5,$resulta[6],1,0,'R');	//body color
		$pdf->SetX(230);
		$pdf->Cell(30,5,$resulta[4],1,0,'R');	//plate number
		$pdf->SetX(260);
		$pdf->Cell(30,5,$resulta[7],1,0,'C');	//lto number
		$pdf->SetX(290);
		$pdf->Cell(30,5,$resulta[10],1,0,'C');	//cr number
		$pdf->SetX(320);
		$pdf->Cell(30,5,$resulta[9],1,0,'C');	//linetype
		$i++;
		$pdf->SetY($pdf->GetY()+5);
	} 
...
$report_desc='Masterlist of Motorized Vehicle By Engine';
