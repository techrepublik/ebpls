...
//Page header
	...
	$this->Cell(340,5,'MASTERLIST OF AMUSEMENT ESTABLISHMENT',0,1,'C');
...
//	$result=mysql_query("select ' ', business_name, ' ' , business_street, ' '  from ebpls_business_enterprise") or die(mysql_error());
	...
	concat(a.business_lot_no, ' ', a.business_street, ' ', a.business_main_offc_barangay_code,' ',
	...
	d.cap_inv, d.bus_nature, d.last_yr 
	...
	a.business_id = d.business_id and d.active=1 and 
	c.active=1 and b.owner_last_name like '$owner_last%' and
	a.business_name like '$buss_name%' and 
	c.transaction like '$trans%' 
	and d.bus_nature like '$bus_nature%'
	and d.cap_inv like '$cap_inv%' and d.last_yr like '$last_yr%'
	and a.business_main_offc_barangay_code like '$brgy_name%'
	") 
	or die(mysql_error());
	$number_of_rows = mysql_numrows($result);
	
	$result2 = mysql_query("select	sum(d.cap_inv)
	from ebpls_business_enterprise a, ebpls_owner b, ebpls_business_enterprise_permit c,
        tempbusnature d
        where b.owner_id = a.owner_id and a.business_id = c.business_id 
	and a.business_id = d.business_id and d.active=1 and c.active=1 
	and b.owner_last_name like '$owner_last%' and
        a.business_name like '$buss_name%' and 
        c.transaction like '$trans%'
        and d.bus_nature like '$bus_nature%'
	and d.cap_inv like '$cap_inv%' and d.last_yr like '$last_yr%'
	...
	$result3 = mysql_query("select  sum(d.last_yr)
        from ebpls_business_enterprise a, ebpls_owner b, ebpls_business_enterprise_permit c,
        tempbusnature d
        where b.owner_id = a.owner_id and a.business_id = c.business_id
        and a.business_id = d.business_id and d.active=1 and c.active=1
	...
	and a.business_main_offc_barangay_code like '$brgy_name%'
	")
        or die("l".mysql_error());
        $resulta3=mysql_fetch_row($result3);
	...
	while ($resulta=mysql_fetch_row($result))
	{
		...
		$pdf->SetX(15);
		$pdf->Cell(25,5,$resulta[0],1,0,'L');
		$pdf->SetX(40);
		$pdf->Cell(55,5,$resulta[3],1,0,'L');
		$pdf->SetX(95);
		$pdf->Cell(60,5,$resulta[1],1,0,'L');
		$pdf->SetX(155);
		$pdf->Cell(60,5,$resulta[5],1,0,'L');
		$pdf->SetX(215);
		$pdf->Cell(60,5,$resulta[2],1,0,'L');
		$pdf->SetX(275);
		$pdf->Cell(30,5,$resulta[4],1,0,'R');
		$pdf->SetX(305);
		$pdf->Cell(30,5,$resulta[6],1,0,'R');
		$i++;
		$pdf->SetY($pdf->GetY()+5);
	} 
...
$pdf->Cell(60,5,'',1,0,'L');
$pdf->SetX(215);
$pdf->Cell(60,5,'Total Investments' ,1,0,'L');
$pdf->SetX(275);
$pdf->Cell(30,5,$resulta2[0],1,0,'R');
$pdf->SetX(305);
$pdf->Cell(30,5,$resulta3[0],1,0,'R');
