...
//Page header
	...
	$this->Cell(340,5,'MASTERLIST OF EXEMPTED BUSINESS ESTABLISHMENT',0,1,'C');
	$this->Ln(22);
	}
...
//if partial  fld '> 0 and fld <100'  else fld '= 100' lagay ka condition dito
$flag = '=100';;
	...
	d.bus_nature, d.cap_inv, d.last_yr, f.tax_exemption, (d.cap_inv * (f.tax_exemption/100)) as totexemp
	from ebpls_business_enterprise a, ebpls_owner b, ebpls_business_enterprise_permit c, 
	tempbusnature d, ebpls_business_category f
	where b.owner_id = a.owner_id and a.business_id = c.business_id and f.tax_exemption $flag
	and a.business_id = d.business_id and f.business_category_code=a.business_category_code") 
	or die(mysql_error());
	$number_of_rows = mysql_numrows($result);
	
	$result2 = mysql_query("select last_yr, f.tax_exemption, (d.last_yr * (f.tax_exemption/100)) as totexemp
	from ebpls_business_enterprise a, ebpls_owner b, ebpls_business_enterprise_permit c,
	 tempbusnature d, ebpls_business_category f
	where b.owner_id = a.owner_id and a.business_id = c.business_id and a.business_id = d.business_id 
	and d.active=1 and f.business_category_code=a.business_category_code and f.tax_exemption $flag") 
	or die(mysql_error());
	...
	while ($resulta=mysql_fetch_row($result))
	{
		...
		$pdf->SetX(325);
		$pdf->Cell(20,5,$resulta[10].'% exempted',1,0,'R');
		$i++;
		$pdf->SetY($pdf->GetY()+5);
	} 
...
$report_desc='Exempted Business Establishment';
