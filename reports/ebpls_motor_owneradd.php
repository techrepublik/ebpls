...
//Page header
	...
	$this->Cell(340,5,'LISTING OF MOTOR VEHICLE BY OWNER AND ADDRESS',0,1,'C');
	$this->Ln(22);
	}
...
$pdf->Cell(30,5,'PLATE NUMBER',1,0,'C');
	...
	concat(a.owner_house_no, ' ', a.owner_street, ' ', a.owner_barangay_code, ' ', a.owner_city_code, ' ', 
	...
	where a.owner_id=b.motorized_operator_id and a.owner_id=c.owner_id order by a.owner_last_name asc") 
	or die(mysql_error());
	...
	while ($resulta=mysql_fetch_row($result))
	{
		...
		$pdf->SetX(55);
		$pdf->Cell(75,5,$resulta[0],1,0,'L');  	//owner
		$pdf->SetX(130);
		$pdf->Cell(70,5,$resulta[12],1,0,'L');	//address
		$pdf->SetX(200);
		$pdf->Cell(30,5,$resulta[1],1,0,'L');	//motor model
		$pdf->SetX(230);
		$pdf->Cell(30,5,$resulta[2],1,0,'L');	//motor #
		$pdf->SetX(260);
		$pdf->Cell(30,5,$resulta[3],1,0,'L');	//chassis color
		$pdf->SetX(290);
...
$report_desc='Masterlist of Motor Vehicle By Owner/Address';
