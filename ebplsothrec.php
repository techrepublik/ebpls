...
$getbus = SelectMultiTable($dbtype,$dbLink,"ebpls_fees_paid",
			"distinct(fee_desc)",
			"where owner_id=$owner_id and
                        permit_type='$permit_type' and permit_status='New' and input_date like '$datengay%' and active='1'");
}
...
$getbus = SelectMultiTable($dbtype,$dbLink,"ebpls_occu_fees",
			"fee_desc, fee_amount",
...
$getbus = SelectMultiTable($dbtype,$dbLink,"ebpls_peddlers_fees",
			"fee_desc, fee_amount",
			"where permit_type='$stat' and active='1'");
...
$getbus = SelectMultiTable($dbtype,$dbLink,"ebpls_fishery_fees",
			"fee_desc, fee_amount",
			"where permit_type='$stat' and active='1'");
$pdf->Cell(75,5,'Boat Registration','0','1');//desc
$pdf->Cell(75,5,'Fish Activity','0','1');//desc
while ($nc<6) {
	while ($getb = FetchRow($dbtype,$getbus))
		{
		$pdf->Cell(75,5,$getb[0],'0','0');//desc
...
if ($permit_type != "Motorized") {
$pdf->Cell(75,5,'Fees ','0','0');//desc
$pdf->Cell(75,5,number_format($getit[1] - ($getotham[amount] + $getotham[bt]),2),'0','1');//amouna
