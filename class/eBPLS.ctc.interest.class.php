...
Module : eBPLS.ctc.interest.class.php
...
class EBPLSCTCInterest extends DataEncapsulator {
	 ...
	 *
	 */
	function EBPLSCTCInterest( $dbLink, $bDebug = false ) {
		...
		$this->addDataElement( CTC_ID, "is_not_empty", "[VALUES]");
	...
	function add(){
		...
		if ( $this->m_dbLink ) {
			if ( ( $error_num = $this->validateData() ) > 0 ) {
				...
				$ret = ebpls_insert_data( $this->m_dbLink, EBPLS_CTC_INTEREST_TABLE, $strValues );
	...
	function view( $owner_id ) {
		...
		$result = ebpls_select_data( $this->m_dbLink, EBPLS_CTC_INTEREST_TABLE, $strValues, $strWhere, NULL, $strOrderBy, "DESC", NULL );
	...
	function update( $industry_id ) {
		...
		if ( ( $error_num = $this->validateData(true) ) > 0 ) {
			...
			$ret = ebpls_update_data( $this->m_dbLink, EBPLS_CTC_INTEREST_TABLE, $strValues, $strWhere );
	...
	function view( $owner_id ) {
		...
		// check if owner have existing transactions
		$result = ebpls_select_data( $this->m_dbLink, EBPLS_CTC_INTEREST_TABLE, $strValues, $strWhere, NULL, NULL, NULL, NULL );
	...
	*/
	...
	function pagesearch($page = 1, $maxrec = 1000000000, $orderkey = CIT_DESC, $is_desc = true ) {
		...
		//echo $is_desc."Robert";
		$result = ebpls_select_data( $this->m_dbLink, EBPLS_CTC_INTEREST_TABLE, $strValues1, $strWhere, NULL, $strOrder, $is_desc, $maxrec, $page  );	
		
		$rowcount = ebpls_select_data( $this->m_dbLink, EBPLS_CTC_INTEREST_TABLE, $strValues2, $strWhere, NULL, $strOrder, $is_desc, NULL, NULL );	
