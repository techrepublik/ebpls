...
Module : eBPLS.economic.class.php
...
define(EBPLS_ECONOMIC_TABLE,"ebpls_economic_area");

// Industry Sector Data Elements Constants
define(ECONOMIC_AREA_ID,"economic_area_id");
define(ECONOMIC_AREA_CODE,"economic_area_code");
define(ECONOMIC_AREA_DESC,"economic_area_desc");
define(UPDATED_BY,"updated_by");
define(DATE_UPDATED,"date_updated");

class EBPLSEconomic extends DataEncapsulator {
	 ...
	 *
	 */
	function EBPLSEconomic( $dbLink, $bDebug = false ) {
		...
		$this->addDataElement( ECONOMIC_AREA_ID, "is_not_empty", "[VALUES]");
		$this->addDataElement( ECONOMIC_AREA_CODE, "is_not_empty", "[VALUES]");
		$this->addDataElement( ECONOMIC_AREA_DESC, "is_not_empty", "[VALUES]");
	...
	function add(){
		...
		if ( $this->m_dbLink ) {
			if ( ( $error_num = $this->validateData() ) > 0 ) {
				...
				$ret = ebpls_insert_data( $this->m_dbLink, EBPLS_ECONOMIC_TABLE, $strValues );
	...
	function view( $owner_id ) {
		...
		$result = ebpls_select_data( $this->m_dbLink, EBPLS_ECONOMIC_TABLE, $strValues, $strWhere, NULL, $strOrderBy, "DESC", NULL );
	...
	function update( $economic_id ) {
		...
		if ( ( $error_num = $this->validateData(true) ) > 0 ) {
			...
			$strWhere[ECONOMIC_AREA_ID] = $economic_id;
	
			$ret = ebpls_update_data( $this->m_dbLink, EBPLS_ECONOMIC_TABLE, $strValues, $strWhere );
	...
	function view( $owner_id ) {
		...
		// check if owner have existing transactions
		$result = ebpls_select_data( $this->m_dbLink, EBPLS_ECONOMIC_TABLE, $strValues, $strWhere, NULL, NULL, NULL, NULL );
	...
	*/
	 ...
	 *
	 */	 
	function search( $economic_area_id = NULL, $economic_area_code = NULL ) {
		
		if ( $economic_area_id != NULL ) {
			$strWhere[ECONOMIC_AREA_ID] = $economic_area_id;
		}
		
		if ( $economic_area_code != NULL ) {
			$strWhere[ECONOMIC_AREA_CODE] = $economic_area_code;
		}
		// select all columns
		$strValues[] = "*";
	...
	function searchedit($new_code, $origid,$tble,$prim_key)
        {
                $tx = mysql_query("select * from $tble where  economic_area_id = '$origid' ");
                $tx = mysql_fetch_array($tx);
                $origid = $tx[economic_area_code];
	...
	function pagesearch($page = 1, $maxrec = 1000000000, $orderkey = ECONOMIC_AREA_CODE, $is_desc = true ) {
		...
		//echo $is_desc."Robert";
		$result = ebpls_select_data( $this->m_dbLink, EBPLS_ECONOMIC_TABLE, $strValues1, $strWhere, NULL, $strOrder, $is_desc, $maxrec, $page  );	
		
		$rowcount = ebpls_select_data( $this->m_dbLink, EBPLS_ECONOMIC_TABLE, $strValues2, $strWhere, NULL, $strOrder, $is_desc, NULL, NULL );	
