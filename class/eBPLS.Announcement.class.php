...
Module : eBPLS.Announcement.class.php
...
define(EBPLS_ANNOUNCEMENT_TABLE,"ebpls_announcement");
...
class EBPLSAnnouncement extends DataEncapsulator {
	...
	function add(){
		...
		if ( $this->m_dbLink ) {
			if ( ( $error_num = $this->validateData() ) > 0 ) {
				...
				$ret = ebpls_insert_data( $this->m_dbLink, EBPLS_ANNOUNCEMENT_TABLE, $strValues );
	...
	function update( $eaid ) {
		...
		if ( ( $error_num = $this->validateData(true) ) > 0 ) {
			...
			$ret = ebpls_update_data( $this->m_dbLink, EBPLS_ANNOUNCEMENT_TABLE, $strValues, $strWhere );
	...
	function delete( $eaid ) {
		...
		$result = ebpls_delete_data ( $this->m_dbLink, EBPLS_ANNOUNCEMENT_TABLE, $strWhere );
	...
	function search( $eaid = NULL, $announcement = NULL ) {
		...
		$result = ebpls_select_data( $this->m_dbLink, EBPLS_ANNOUNCEMENT_TABLE, $strValues, $strWhere, NULL, NULL, NULL, NULL);	
		
		$rowcount = ebpls_select_data( $this->m_dbLink, EBPLS_ANNOUNCEMENT_TABLE, $strValues1, $strWhere, NULL, NULL, NULL, NULL);	
	...
	function pagesearch($page = 1, $maxrec = 1000000000, $orderkey = EAID, $is_desc = true ) {
		...
		//echo $is_desc."Robert";
		$result = ebpls_select_data( $this->m_dbLink, EBPLS_ANNOUNCEMENT_TABLE, $strValues1, $strWhere, NULL, $strOrder, $is_desc, $maxrec, $page  );	
		
		$rowcount = ebpls_select_data( $this->m_dbLink, EBPLS_ANNOUNCEMENT_TABLE, $strValues2, $strWhere, NULL, $strOrder, $is_desc, NULL, NULL );	
